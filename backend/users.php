                        ...
                        <div class="row">
                            <div class="col-xl-4">
                                <div class="card widget widget-list">
                                    <?php 
                                    $db_users_query = "SELECT id, name, email FROM users;";
                                    $db_users_count_query = "SELECT COUNT(*) AS 'result' FROM users;";
                                    $final_users_slesction = mysqli_query($db_connect, $db_users_query);
                                    $final_users_count_slesction = mysqli_query($db_connect, $db_users_count_query);
                                    $user_count_assoc = mysqli_fetch_assoc($final_users_count_slesction);
                                    // print_r($user_assoc);
                                    $_SESSION['u_result_list'] = $user_count_assoc['result'];
                                    ?>
                                    <div class="card-header">
                                        <h5 class="card-title">Active Users<span class="badge badge-success badge-style-light">Total: <?=$_SESSION['u_result_list'] ?> </span></h5>
                                    </div>
                                    <div class="card-body" style="overflow-y: scroll; height: 410px;">
                                        <span class="text-muted m-b-xs d-block">showing 5 out of <?=$_SESSION['u_result_list'] ?> active users.</span>
                                        <ul class="widget-list-content list-unstyled">
                                            <?php foreach ($final_users_slesction as $users):
